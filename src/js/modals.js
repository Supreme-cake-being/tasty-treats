...
export const renderModals = data => {
  ...
  if (data.tags.length === 0) {
    tags[0] = '<li class="modal-tag-item">#No tags yet</li>';
  } else {
...
refs.recipe.addEventListener('click', evt => {
  if (
    ...
    evt.target.classList.contains('card-favorites-btn')
  )
    return;
...
refs.overlay.addEventListener('click', evt => {
  ...
  if (evt.target.type === 'button') {
    if (
      evt.target.classList.value === 'modal-fav-btn' &&
      evt.target.attributes.favorites.value === '1'
    ) {
      refs.favBtn.textContent = 'Add to favorite';
      refs.favBtn.setAttribute('favorites', 0);
    } else {
...
rating.addEventListener('change', evt => {
  rat.textContent = evt.target.value;
});
...
refs.submitRatingBth.addEventListener('click', evt => {
  ...
  if (refs.submitRatingEmail.value.match(emailFormat)) {
    console.log(
      `{ID: "${refs.ratingBtn.attributes.id.value}" , email: ${refs.submitRatingEmail.value}, rating: ${refs.submitRecipeRating.textContent}}`
    );
    closeModalRating();
  } else {
